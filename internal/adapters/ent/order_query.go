...
func (oq *OrderQuery) QueryLaunches() *LaunchQuery {
	query := &LaunchQuery{config: oq.config}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withLaunches: oq.withLaunches.Clone(),
...
func (oq *OrderQuery) WithLaunches(opts ...func(*LaunchQuery)) *OrderQuery {
	query := &LaunchQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
	oq.withLaunches = query
	return oq
}
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withLaunches; query != nil {
		if err := oq.loadLaunches(ctx, query, nodes,
			func(n *Order) { n.Edges.Launches = []*Launch{} },
			func(n *Order, e *Launch) { n.Edges.Launches = append(n.Edges.Launches, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadLaunches(ctx context.Context, query *LaunchQuery, nodes []*Order, init func(*Order), assign func(*Order, *Launch)) error {
	...
	query.Where(predicate.Launch(func(s *sql.Selector) {
		s.Where(sql.InValues(order.LaunchesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.order_launches
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_launches" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_launches" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
