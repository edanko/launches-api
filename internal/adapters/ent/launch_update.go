...
func (lu *LaunchUpdate) SetFiles(s []string) *LaunchUpdate {
	lu.mutation.SetFiles(s)
	return lu
}

// ClearFiles clears the value of the "files" field.
func (lu *LaunchUpdate) ClearFiles() *LaunchUpdate {
...
func (lu *LaunchUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := lu.mutation.Files(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
			Column: launch.FieldFiles,
		})
	}
...
// SetFiles sets the "files" field.
func (luo *LaunchUpdateOne) SetFiles(s []string) *LaunchUpdateOne {
	luo.mutation.SetFiles(s)
	return luo
}

// ClearFiles clears the value of the "files" field.
func (luo *LaunchUpdateOne) ClearFiles() *LaunchUpdateOne {
...
func (luo *LaunchUpdateOne) Save(ctx context.Context) (*Launch, error) {
	...
	if len(luo.hooks) == 0 {
		...
		nv, ok := v.(*Launch)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from LaunchMutation", v)
		}
		node = nv
	}
	return node, err
...
func (luo *LaunchUpdateOne) sqlSave(ctx context.Context) (_node *Launch, err error) {
	...
	if value, ok := luo.mutation.Files(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeJSON,
			Value:  value,
			Column: launch.FieldFiles,
