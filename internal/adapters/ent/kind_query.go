...
func (kq *KindQuery) GroupBy(field string, fields ...string) *KindGroupBy {
	grbuild := &KindGroupBy{config: kq.config}
...
func (kq *KindQuery) Select(fields ...string) *KindSelect {
	...
	selbuild := &KindSelect{KindQuery: kq}
	selbuild.label = kind.Label
...
func (kq *KindQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Kind, error) {
	var (
		nodes       = []*Kind{}
		_spec       = kq.querySpec()
		loadedTypes = [1]bool{
	...
	_spec.ScanValues = func(columns []string) ([]interface{}, error) {
		return (*Kind).scanValues(nil, columns)
	}
	...
	if query := kq.withLaunches; query != nil {
		if err := kq.loadLaunches(ctx, query, nodes,
			func(n *Kind) { n.Edges.Launches = []*Launch{} },
			func(n *Kind, e *Launch) { n.Edges.Launches = append(n.Edges.Launches, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (kq *KindQuery) loadLaunches(ctx context.Context, query *LaunchQuery, nodes []*Kind, init func(*Kind), assign func(*Kind, *Launch)) error {
	...
	for _, n := range neighbors {
		fk := n.kind_launches
		if fk == nil {
			return fmt.Errorf(`foreign-key "kind_launches" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "kind_launches" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
