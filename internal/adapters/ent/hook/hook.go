...
import (
	...
	"github.com/edanko/launches-api/internal/adapters/ent"
)
...
// The LaunchFunc type is an adapter to allow the use of ordinary
// function as Launch mutator.
type LaunchFunc func(context.Context, *ent.LaunchMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f LaunchFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.LaunchMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LaunchMutation", m)
	}
	return f(ctx, mv)
}
