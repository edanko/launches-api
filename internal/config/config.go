...
type Config struct {
	...
	LDAP struct {
		Host        string `yaml:"host" env:"LDAP_HOST" env-default:"localhost" env-required:"true"`
		Port        int    `yaml:"port" env:"LDAP_PORT" env-default:"389" env-required:"true"`
		User        string `yaml:"user" env:"LDAP_USER" env-required:"true"`
		Password    string `yaml:"password" env:"LDAP_PASSWORD" env-required:"true"`
		BaseDN      string `yaml:"baseDN" env:"LDAP_BASE_DN" env-required:"true"`
		FilterUsers string `yaml:"filterUsers" env:"LDAP_FILTER_USERS" env-required:"true"`
